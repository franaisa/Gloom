...
static const float  BRIGHT_PASS_THRESHOLD  = 1.5f;  // Threshold for BrightPass filter
...
float	g_fInvWhitePointSq;		// 1 / (white^2)
	...
	) : COLOR
{
	...
	if( g_bEnableToneMap )
	{
		...
		vSample.rgb = vSample.rgb / (1.0f+vSample.rgb);
//		vSample.rgb = (vSample.rgb * (1.0f + vSample.rgb*g_fInvWhitePointSq)) / (1.0f+vSample.rgb);
	}
