...
void computeLighting(float4 lightPos, float3 lightDir, float4 lightCol, float3 P, float3 N, float3 eyePos, float shin, float4 att, out float3 diffuseResult, out float3 specularResult) {
	...
	diffuseResult = lightCol.xyz * diffuseLight;
	...
	specularResult = lightCol.xyz * specularLight;
}
...
float4 fragment_main(const PsInput IN) : COLOR {
	...
	//for (int i = 0; i < 1; ++i) {
		computeLighting(lightPosition, -lightDirection, lightColor, P, N, eyePosition, shininess, lightAttenuation, diffuseLight, specularLight);
