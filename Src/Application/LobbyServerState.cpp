...
namespace Application {
	...
	Hikari::FlashValue CLobbyServerState::createReleased(Hikari::FlashControl* caller, const Hikari::Arguments& args)
	{
		...
		if (!Logic::CEntityFactory::getSingletonPtr()->loadBluePrints("archetypes.xml", "Server")) {
		//if (!Logic::CEntityFactory::getSingletonPtr()->loadBluePrints(_map+"", "Server")) {
			Net::CManager::getSingletonPtr()->deactivateNetwork();
			_app->exitRequest();
		}
		
		if (!Logic::CEntityFactory::getSingletonPtr()->loadArchetypes("archetypes.xml", "Server")) {
			Net::CManager::getSingletonPtr()->deactivateNetwork();
			_app->exitRequest();
		}
		...
		if (!Logic::CServer::getSingletonPtr()->loadLevel(_map + ".xml","Server")) {
			Net::CManager::getSingletonPtr()->deactivateNetwork();
			_app->exitRequest();
		}
