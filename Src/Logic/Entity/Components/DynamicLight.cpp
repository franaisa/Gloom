...
namespace Logic {
	...
	void CDynamicLight::turnOn(const Vector3& offset, float timeToLive) {
		...
		CLightManager::getSingletonPtr()->createLight(_light, _lightType, _lightGroup, _static, this, _controlledByManager, _entity->getPosition() + _offset, -_entity->getOrientation().zAxis());
