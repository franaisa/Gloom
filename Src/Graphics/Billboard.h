...
Contiene la declaración de la clase que representa un billboard.
...
namespace Graphics {
	...
	*/
	...
	public:
		...
		*/
		CBillboard(const std::string& bbSetName, const Ogre::Vector3& position);
		...
		*/
		...
		void addMaterial(const std::string& materialName, const Ogre::Vector3& position, float width = 0, float height = 0);
		...
		*/
		void addText(const std::string& text, const Ogre::Vector3& position, const Ogre::ColourValue& color, 
					 const std::string& fontName = "Verdana", float width = 0, float height = 0);

	protected:

		void WriteToTexture(const Ogre::String &str, Ogre::Texture* destTexture, 
							Ogre::Image::Box destRectangle, Ogre::Font* font, 
							const Ogre::ColourValue& color, char justify = 'l',
							bool wordwrap = true);
